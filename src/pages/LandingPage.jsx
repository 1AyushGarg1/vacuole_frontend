...
import { ArrowRight, Sparkles, Search, Heart, Filter, ShoppingBag, Star, Zap, Target, BookOpen, TrendingUp, Shield, Clock, Users, Database, Cpu, ChevronUp, Package, Store, Smartphone, Shirt, Headphones, Laptop, Sun, Moon } from 'lucide-react';
import {
  UserButton,
  useClerk,
  useUser
...
export default function LandingPage({ theme, onThemeChange }) {
  ...
  const themeClasses = {
    dark: {
      bg: "min-h-screen bg-gray-900 text-white",
      navBg: "backdrop-blur-md bg-gray-900/90 border-b border-gray-800/50",
      cardBg: "bg-gray-800/60 border border-gray-600/40",
    ...
    light: {
      bg: "min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 text-slate-900",
      navBg: "backdrop-blur-md bg-white/90 border-b border-slate-200/50",
      cardBg: "bg-white/80 border border-slate-200/60",
  ...
  return (
    <div className={`${currentTheme.bg} overflow-hidden relative`}>
      {/* Enhanced Animated background with parallax - adjusted for theme */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div 
          className={`absolute w-96 h-96 ${theme === 'dark' ? 'bg-gradient-to-r from-green-400/10 to-emerald-400/5' : 'bg-gradient-to-r from-emerald-400/8 to-emerald-400/4'} rounded-full blur-3xl`}
          style={{
        ...
        ></div>
        <div 
          className={`absolute w-64 h-64 ${theme === 'dark' ? 'bg-gradient-to-r from-emerald-400/15 to-green-500/8' : 'bg-gradient-to-r from-emerald-400/12 to-emerald-500/6'} rounded-full blur-2xl`}
          style={{
        ...
        ></div>
        <div 
          className={`absolute w-48 h-48 ${theme === 'dark' ? 'bg-gradient-to-r from-green-300/12 to-emerald-500/6' : 'bg-gradient-to-r from-emerald-300/10 to-emerald-500/5'} rounded-full blur-xl`}
          style={{
        ...
        <div className={`absolute top-1/4 right-1/4 w-32 h-32 ${theme === 'dark' ? 'bg-green-400/5' : 'bg-emerald-400/4'} rounded-full blur-2xl animate-pulse`}></div>
        <div className={`absolute bottom-1/3 left-1/3 w-40 h-40 ${theme === 'dark' ? 'bg-emerald-400/8' : 'bg-emerald-400/6'} rounded-full blur-3xl animate-bounce delay-1000`}></div>
      </div>
      <style jsx>{`
        @keyframes float {
      ...
      {/* Navigation */}
      <nav className={`relative z-50 px-6 py-4 ${currentTheme.navBg}`}>
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className={`p-2 rounded-lg transition-all duration-300 ${
                theme === 'dark' 
                  ? 'bg-gray-800/50 text-gray-300 hover:bg-gray-700/50 hover:text-emerald-300' 
                  : 'bg-slate-100/50 text-slate-600 hover:bg-slate-200/50 hover:text-emerald-600'
              }`}
      ...
      <div className="relative min-h-screen flex items-center justify-center px-6">
        <div className={`max-w-6xl mx-auto text-center relative z-10 transition-all duration-1500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="mb-8">
            <span className={`inline-flex items-center px-6 py-3 rounded-full text-sm ${theme === 'dark' ? 'bg-gray-800/60 border border-green-400/40' : 'bg-white/60 border border-emerald-400/40'} backdrop-blur-md text-green-400 shadow-lg`}>
          ...
          <h1 className="text-6xl md:text-5xl font-bold mb-10 leading-tight">
            <span className={`${theme === 'dark' ? 'bg-gradient-to-r from-white via-gray-100 to-gray-200' : 'bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700'} bg-clip-text text-transparent`}>
          ...
          <p className={`text-2xl md:text-2xl ${currentTheme.textSecondary} mb-16 max-w-5xl mx-auto leading-relaxed`}>
          ...
          <div className="mb-20">
            <p className={`${currentTheme.textMuted} mb-10 text-xl`}>Works seamlessly with 50+ platforms</p>
            <div className={`relative overflow-hidden ${theme === 'dark' ? 'bg-gray-800/20 border border-gray-700/30' : 'bg-white/20 border border-slate-200/30'} rounded-2xl py-8 backdrop-blur-sm`}>
              <div className="flex space-x-12 scrolling-logos">
                {[...ecommerceLogos, ...ecommerceLogos].map((platform, index) => (
                  <div 
                    key={`${platform.name}-${index}`}
                    className={`flex-shrink-0 group flex items-center space-x-4 px-8 py-4 rounded-xl ${theme === 'dark' ? 'bg-gray-800/40 border border-gray-600/40 hover:border-green-400/60 hover:bg-gray-700/40' : 'bg-white/40 border border-slate-200/40 hover:border-emerald-400/60 hover:bg-slate-50/40'} backdrop-blur-sm transition-all duration-500 hover:scale-110 cursor-pointer`}
                  >
                    ...
                    <span className={`${currentTheme.textSecondary} font-semibold text-lg group-hover:text-green-400 transition-colors duration-300 whitespace-nowrap`}>
                      {platform.name}
                    </span>
                  </div>
                ))}
      ...
      <div className="py-40 px-6 relative">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-24">
            ...
            <p className={`text-3xl ${currentTheme.textSecondary} max-w-4xl mx-auto leading-relaxed`}>
            ...
            {features.map((feature, index) => (
              ...
              >
                <div className="flip-card-inner">
                  <div className={`flip-card-front p-10 ${currentTheme.cardBg} backdrop-blur-md ${currentTheme.cardHover} transition-all duration-700 transform hover:scale-105 hover:shadow-2xl hover:shadow-green-400/20 flex flex-col justify-center items-center text-center relative overflow-hidden`}>
                    ...
                    <p className={`${currentTheme.textSecondary} leading-relaxed text-xl mb-6 relative z-10`}>
                      {feature.shortDesc}
                    </p>
                  ...
                  <div className="flip-card-back p-8 bg-gradient-to-br from-green-400/15 to-emerald-500/10 border border-green-400/40 backdrop-blur-md">
                    <div className="h-full flex flex-col">
                      ...
                      <p className={`${theme === 'dark' ? 'text-gray-200' : 'text-slate-100'} text-base mb-8 leading-relaxed flex-grow`}>
                      ...
                      <div className="space-y-3">
                        {feature.fullContent.benefits.map((benefit, i) => (
                          <div key={i} className="flex items-center space-x-3 text-base">
                            ...
                            <span className={`${theme === 'dark' ? 'text-gray-200' : 'text-slate-100'} font-medium`}>{benefit}</span>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      <div className={`py-40 px-6 ${currentTheme.sectionBg} relative`}>
          ...
          <div className="grid md:grid-cols-4 gap-16">
            {[
              ...
              <div key={index} className="relative group">
                ...
                <p className={`${currentTheme.textSecondary} leading-relaxed text-lg`}>{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="py-20 px-4 ">
        <div className="max-w-6xl mx-auto text-center">
          <div className={`p-20 rounded-3xl ${currentTheme.ctaBg} backdrop-blur-md relative overflow-hidden shadow-2xl`}>
            ...
            <div className="relative z-10">
              ...
              <p className={`text-2xl ${theme === 'dark' ? 'text-gray-200' : 'text-slate-700'} mb-16 max-w-4xl mx-auto leading-relaxed`}>
              ...
              <button className={`border-2 ${theme === 'dark' ? 'border-gray-500 hover:border-green-400 hover:bg-green-400/15 hover:text-green-400' : 'border-slate-400 hover:border-emerald-400 hover:bg-emerald-400/15 hover:text-emerald-600'} px-8 py-4 rounded-full text-lg font-semibold transition-all duration-500 flex items-center space-x-3 group hover:scale-105`}>
      ...
      <footer className={`py-16 px-6 ${currentTheme.footerBg} backdrop-blur-md relative`}>
        <div className="max-w-6xl mx-auto">
          <div className="text-center">
            ...
            <p className={`${currentTheme.textSecondary} text-xl mb-8 max-w-2xl mx-auto`}>
